...
void encode(RSEncoder& enc, const char* in_path, const char* out_path) {
	...
	while (!input.eof()) {
		padding += read(input, message, enc.maxLength());
		enc.encode(message, encbuf);
		printf("encoded block %d (%d bytes)\n", i, message.size());
		i++;
	}

	output << padding;
	std::cout << "Added " << padding << " bytes padding" << std::endl;
